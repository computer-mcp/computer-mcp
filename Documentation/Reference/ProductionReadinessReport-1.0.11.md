# Computer MCP 1.0.11 Production Readiness Report
...
| Authenticated observation provenance | Audit-derived connection, caller, Tunnel instance, and Tunnel profile identity with fail-closed regressions |
...
A physical Intel Mac is not a 1.0.11 release gate. Universal 2 slice checks and
...
PASS must bind consumer result, authenticated transport or Tunnel instance,
Gateway request ID, one audit row, result digest, and cleanup digest to this
candidate identity. The exact-artifact `config.export` inventory must match the
effective authenticated catalog, including persisted Full Shell and workspace
grants. OpenAI acceptance uses a fresh Connector scan and a new ChatGPT
session. Cloudflare named-tunnel acceptance uses a test or user-owned account,
hostname, and runtime token; no Cloudflare credential belongs to the product
or release workflow, and Quick Tunnel evidence cannot substitute for this
case.
