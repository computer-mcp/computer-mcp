# Computer MCP 1.0.15 Production Readiness Report
...
| Workspace canonical containment | Existing ancestors are resolved before authorization; escaping symlink parents are denied for built-in tools and operation-state tickets |
| Workspace escape regressions | Direct missing-target writes, symlink destinations, and `operations.prepare` through an escaping symlink are all denied without an outside mutation |
| Recursive symlink scan boundary | Directory entries preserve the final symlink identity; read-only recursive scans never follow symlink targets and tolerate dangling links without weakening explicit-path containment |
...
| Authenticated runtime observation contract | OpenAI Secure MCP Tunnel evidence preserves the observed instance, profile, and socket identities and remains consumer-free |
...
| Gateway provider lifecycle | Disconnect and stop regressions release App Server, Exec, MCP, HTTP, stdio, workspace, and downstream resources; the repeatable authenticated probe covers drift denial, cancellation, and crash recovery |
...
A physical Intel Mac is not a 1.0.15 release gate. Universal 2 slice checks and
