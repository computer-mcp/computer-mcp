# Computer MCP 1.0.17 Production Readiness Report
...
| App/CLI package and version boundary | 1.0.17 (18), passed by the bound run |
| Validation canonical catalog | 22 publisher-verifiable Test Cases with exact ID equality |
| Workspace selection | Native `NSOpenPanel` on every supported macOS release; directory-only selection enters the persistent-bookmark path and Workspaces/Home refresh in place |
| Local registration boundary | Folder selection and bookmark persistence are local App operations, independent of website, provider, anti-scraping, Shell, and Cloudflare decisions |
...
| --- | --- | --- |
| Apple Silicon, macOS 14+ | DMG cold install, Finder launch, native workspace panel, TCC/state/CLI lifecycle, and exact catalog | Required before draft publication |
...
A physical Intel Mac is not a 1.0.17 release gate. Universal 2 slice checks and
...
Workspace **Add** must open the native macOS directory panel, accept a newly
created fixture directory, persist a non-stale bookmark, and update the
existing Workspaces page without navigation or a full-page replacement. This
is a local UI and storage gate; provider or website responses cannot satisfy or
invalidate it.
...
The immutable `v1.0.16` tag remains a rejected candidate record. Its run was
canceled before draft creation after macOS 27 UI acceptance exposed the
separate directory-browser failure; no 1.0.16 Release exists and none of its
assets may be reused for this acceptance.
