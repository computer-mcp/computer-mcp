# Computer MCP 1.0.23 Production Readiness Report
...
| App/CLI package and version boundary | 1.0.23 (24), passed by the bound run |
...
| Codex process lifecycle | Owned process-group identity and bounded EOF/TERM/KILL/reap across close, replacement, timeout, shutdown, parent death, and stubborn descendants |
...
| Menu-bar identity | Native `MenuBarExtra` renders the Computer MCP title with the live service-state icon |
| Window lifecycle | Closing the control center preserves the status item, gateway, and managed tunnels |
| Stable security identity | Bundle ID, Team, Keychain group, TCC identity, and production runtime namespace are unchanged |
| Workspace selection | Native `NSOpenPanel`; directory selection enters the persistent-bookmark path and refreshes in place |
...
| Read-only provider retry | Only a deadline-expired read is retried once on a fresh connection; writes and other failures are not retried |
| Non-cooperative timeout bound | The losing RPC and transport close cannot delay the deadline; the connection retires before retry |
...
A physical Intel Mac is not a 1.0.23 release gate. Universal 2 slice checks and
...
Publishing the existing GitHub draft and enabling the reviewed Pages deployment
...
replaced between acceptance and publication.
