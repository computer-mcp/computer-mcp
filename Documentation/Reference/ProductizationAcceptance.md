...
| --- | --- | --- |
| O1, B1–B11 | `CodexAppServerProcessTransport.swift`; lifecycle/process-tree, two-workspace, 256 KiB long-lived-response, and immediate-exit final-line tests; serialized and awaited available-chunk stdout framing; one end-to-end request budget covering startup, validation, RPC, and the sole read-only retry; shared bounded retirement; gated `RealCodexAppServerAcceptanceTests` proving official `skills/list`, bounded `app/list`, teardown, and the two-client resume/archive lifecycle | Implemented; focused lifecycle 33/33, transport 7/7, complete serial 52 suites/797 tests, and real official-client 3/3 reruns passed for the 1.0.25 source candidate |
...
| Swift regression | `/usr/bin/swift test --no-parallel`: 52 suites/797 tests passed on 2026-09-02. The supported-default build and strict format/lint gates also passed. The 1.0.25 delta's focused transport 7/7, lifecycle 33/33, and all three gated official-client tests passed, including `skills/list` in 0.582 seconds, bounded `app/list`, and two-client resume/archive | Local 1.0.25 source and real-client gates passed; exact signed-tag CI remains required |
...
| W1–W11 | Independent sibling Git repository `computer-mcp.github.io`; static product site, responsive control-path design, metadata/social assets, no `CNAME`, Vite build, HTML/link checks, Axe, desktop/mobile Playwright, CI, and Pages workflow; signed commit `0657fcda80247ef755f30d633aee083f4fe2634f` binds `/release.json` to 1.0.25 and `v1.0.25` | Clean website gate passed: production build and all 16 links passed; 13 of 14 browser scenarios passed and the desktop-only mobile-menu scenario skipped by design; repository remains private with deployment disabled until the coordinated publication decision |
| Q1–Q12 | Strict format/lint, supported-default root and Validation builds, 52 suites/797 root tests, 11 suites/67 Validation tests, 22 canonical Test Case definitions, real Codex acceptance, development Universal 2 App/DMG verification, documentation and release-boundary regressions, and website gates | Partial: the reviewed 1.0.25 source candidate and website commit are signed and clean; protected signed-tag CI, notarized 1.0.25 artifacts, exact-artifact 22/22 acceptance, the new-conversation ChatGPT check, and coordinated publication remain required |
| CS1–CS10 | Independent 1.0.25 cold-start audit passed 10/10 against reviewed source parent `32c47d33b10ff49f4cd4578ee6c36b6278d4d0c8` and website `0657fcda80247ef755f30d633aee083f4fe2634f`; root 52 suites/797 tests plus real Codex 3/3, focused lifecycle 33/33, transport 7/7, Validation 11/67, and catalog 22/22 passed; simulated external-writer and durable-runtime-receipt diagnosis passed 2/2; a bounded `workspace.source_files` WGSL classification update passed its focused test and was cleanly reverted; website build, 16/16 links, and Playwright 13 passed plus 1 expected desktop skip; release procedure, stable/experimental/planned boundaries, deterministic metadata 13/28/41, and both clean worktrees were verified | Passed: the 1.0.25 source cold-start audit is complete; protected signed-tag CI, notarized exact-artifact acceptance, the new-conversation ChatGPT check, and coordinated publication remain open |
