...
| --- | --- | --- |
| O1, B1–B11 | `CodexAppServerProcessTransport.swift`; lifecycle/process-tree, two-workspace, and 256 KiB long-lived-response tests; available-chunk stdout framing; one end-to-end request budget covering startup, validation, RPC, and the sole read-only retry; shared bounded retirement; gated `RealCodexAppServerAcceptanceTests` proving official `skills/list`, bounded `app/list`, teardown, and the two-client resume/archive lifecycle | Implemented; focused lifecycle 33/33, transport 6/6, complete serial 52 suites/796 tests, and real official-client 3/3 reruns passed for the 1.0.25 source candidate |
...
| Swift regression | `/usr/bin/swift test --build-system native --no-parallel`: 52 suites/796 tests passed on 2026-09-02. The supported-default build and serial test gate also passed. The 1.0.25 delta's focused transport 6/6, lifecycle 33/33, and all three gated official-client tests passed, including `skills/list` in 0.365 seconds, bounded `app/list`, and two-client resume/archive | Local 1.0.25 source and real-client gates passed; exact signed-tag CI remains required |
...
| Q1–Q12 | Strict format/lint, supported-default root and Validation builds, 52 suites/796 root tests, 11 suites/67 Validation tests, 22 canonical Test Case definitions, real Codex acceptance, development Universal 2 App/DMG verification, documentation and release-boundary regressions, and website gates | Partial: local source and website gates pass; final main commit, clean signed-tag CI, notarized 1.0.25 artifacts, exact-artifact 22/22 acceptance, new-conversation ChatGPT check, final clean worktrees, and coordinated publication remain required |
| CS1–CS10 | Fresh Codex task `01a05e6f-e018-7711-94ce-3175e029f926` passed 10/10 against audited parent `faf1377a1f05a2ee586f72d1b83cd506437e1ea8` and website `1ac2843ab55bbee1e479797fd365e703b342d57e`; root 52 suites/794 tests plus real Codex 2/2, Validation 11/67, and catalog 22/22; fresh no-reuse Universal 2 App/CLI/DMG 1.0.24 (25), arm64 + x86_64, source-bound to the parent; website 16/16 links and Playwright 13 passed + 1 expected desktop skip; deprecated build flag, plist-derived release names, public-worktree hygiene, both SwiftPM graph shapes (13/28/41), and owner-death 10×/50-case closures passed; both worktrees were clean | Historical 1.0.24 audit passed; an independent defect-first review and cold-start delta audit remain required for 1.0.25 before release |
