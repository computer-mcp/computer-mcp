...
| --- | --- | --- |
| O1, B1–B11 | `CodexAppServerProcessTransport.swift`; lifecycle/process-tree and two-workspace tests; gated `RealCodexAppServerAcceptanceTests` completing a real turn, reaping the first official client, resuming and archiving from a second client, then reaping it | Implemented; focused, full-suite, and final real-client reruns passed |
...
| Swift regression | `/usr/bin/swift test --build-system native --no-parallel`: 52 suites, 789 tests passed on 2026-09-01; the gated real official-client acceptance also passed separately | Final local code and real-client gates passed |
...
| W1–W11 | Independent sibling Git repository `computer-mcp.github.io`; static product site, responsive control-path design, metadata/social assets, no `CNAME`, Vite build, HTML/link checks, Axe, desktop/mobile Playwright, CI, and Pages workflow | Final cold-start website check passed: production build and all 16 links passed; 13 browser tests passed, the desktop-only mobile-menu case skipped by design, and clipboard success and denial fallback paths were both exercised |
| Q1–Q12 | Strict format/lint, native root and Validation builds, 789 root tests, 67 Validation tests, 22 canonical Test Cases, real Codex acceptance, development Universal 2 App/DMG verification, documentation, release-boundary regressions, and website gates; main commits `fdd5739` and `487879a`; website commits `eb52a16` and `3cb723a` | Q1–Q12 passed for the coordinated release-ready batch; both repositories are clean and no partial publication occurred |
| CS1–CS10 | Fresh Codex task `01a05d02-e414-7673-a4c1-3126b5de3d61` started from repository evidence, passed CS1–CS8 and CS10, found the clipboard-denial cold-start defect in CS9, and cleanly reverted its CS8 audit-only change; after the website fix it reran the unchanged complete gate and verified both repositories clean | Passed 10/10 without the original conversation; main at `487879a`, website at `3cb723a` |
