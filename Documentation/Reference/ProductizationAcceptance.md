...
| --- | --- | --- |
| O1, B1–B11 | `CodexAppServerProcessTransport.swift`; lifecycle/process-tree and two-workspace tests; one end-to-end request budget covering startup, validation, RPC, and the sole read-only retry; shared bounded retirement; gated `RealCodexAppServerAcceptanceTests` proving official `app/list` respects the 30-second call budget plus teardown, then completing the two-client resume/archive lifecycle | Implemented; focused, complete serial, and real official-client reruns passed for the 1.0.24 source candidate |
...
| Swift regression | `/usr/bin/swift test --no-parallel`: 52 suites, 794 tests passed on 2026-09-02; the two gated real official-client tests passed separately, including official `app/list` in 34.027 seconds with a 30-second request budget and bounded process teardown | Local 1.0.24 source and real-client gates passed; exact signed-tag CI remains required |
