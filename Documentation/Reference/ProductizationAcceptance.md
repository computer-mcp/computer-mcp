...
| Swift regression | `/usr/bin/swift test --build-system native --no-parallel`: 52 suites, 794 tests passed on 2026-09-02; the two gated real official-client tests passed separately, including official `app/list` in 34.027 seconds with a 30-second request budget and bounded process teardown | Local 1.0.24 source and real-client gates passed; exact signed-tag CI remains required |
...
| Q1–Q12 | Strict format/lint, native root and Validation builds, 794 root tests, 67 Validation tests, 22 canonical Test Case definitions, real Codex acceptance, development Universal 2 App verification, documentation and release-boundary regressions, and website gates; website commit `1ac2843` | Partial: local source and website gates pass, but the final main commit, clean signed-tag CI, notarized 1.0.24 artifacts, exact-artifact 22/22 acceptance, new-conversation ChatGPT check, final clean worktrees, and coordinated publication remain required |
| CS1–CS10 | Earlier fresh Codex task `01a05d02-e414-7673-a4c1-3126b5de3d61` passed 10/10 from repository evidence and found the website clipboard-denial defect that was fixed before the 1.0.24 binding | Partial: useful independent evidence exists, but a fresh cold-start audit against the final 1.0.24 main and website commits remains required |
