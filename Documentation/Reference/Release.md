...
Example after the repository version has been changed to `1.0.9`:

```sh
git switch master
git pull --ff-only origin master
git tag -s -a v1.0.9 -m "Computer MCP 1.0.9"
...
spctl --assess --type open --context context:primary-signature --verbose=2 \
  Computer-MCP-1.0.9-universal.dmg
xcrun stapler validate Computer-MCP-1.0.9-universal.dmg
```
