# Computer MCP 1.0.13 Release Notes
...
- Codex App Server, Exec, and MCP child processes now follow the active fixed
  macOS HTTP, HTTPS, and SOCKS proxies when Computer MCP is launched normally
  from Finder or at login. This closes the production-only path where Safari
  and interactive Codex worked but App Server `apps/list` returned HTTP 403.
- An explicitly inherited proxy environment takes precedence. Computer MCP
  mirrors conventional uppercase and lowercase variables for compatibility,
  keeps loopback and macOS bypass hosts in `NO_PROXY`, and does not evaluate
  proxy auto-configuration scripts.
- Resolved proxy values are derived child-process state. They are not written
  to the manifest, Keychain, configuration export, logs, evidence, DMG, or
  GitHub Release, and system-derived URLs contain no credentials.
- Secure Tunnel runtime evidence continues to require a complete audit-derived
  Tunnel instance, Tunnel profile, Gateway Socket connection, and stable
  schema-2 result classification.
...
1. Download `Computer-MCP-1.0.13-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
