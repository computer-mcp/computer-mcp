# Computer MCP 1.0.14 Release Notes
...
- Workspace containment now resolves the deepest existing ancestor before
  authorizing a path. A missing write destination below a symlink that exits
  the registered workspace is denied before an operation ticket is issued.
- Built-in tools and ticket preparation share the same canonical path rule.
  Direct file writes, symlink destinations, and operation-state binding have
  dedicated regressions, and boundary rejections are audited as denials.
- Runtime validation now preserves the authenticated OpenAI Secure MCP Tunnel
  instance, profile, and socket identity instead of describing those calls as
  an unauthenticated local socket. The repeatable downstream probe also seals
  drift denial, cancellation propagation, and crash recovery evidence.
- The notarized 1.0.13 draft remains unpublished because exact-artifact
  acceptance reproduced the escaping-symlink write. Users never received that
  candidate; 1.0.14 repeats signing, notarization, installation, and external
  acceptance from the corrected source.
...
1. Download `Computer-MCP-1.0.14-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
