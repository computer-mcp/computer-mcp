# Computer MCP 1.0.20 Release Notes
...
- Validation-owned Codex threads are archived through their selected fixture
  workspace, so exact-catalog cleanup remains deterministic when multiple
...
1. Download `Computer-MCP-1.0.20-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
...
- Replacing 1.0.18 with 1.0.20 preserves the production runtime namespace and
