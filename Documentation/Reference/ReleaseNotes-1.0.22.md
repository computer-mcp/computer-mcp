# Computer MCP 1.0.22 Release Notes
...
- A Codex App Server deadline no longer waits for the losing RPC task or its
  transport close to cooperate with cancellation. The timed-out connection is
  retired before the caller resumes and closes asynchronously.
...
- The immutable signed 1.0.21 candidate remains unpublished. Exact-artifact
  acceptance found that its structured timeout race still waited for a
  non-cooperative request and connection close after the deadline.

## Install

1. Download `Computer-MCP-1.0.22-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
...
- Replacing 1.0.18 with 1.0.22 preserves the production runtime namespace and
