# Computer MCP 1.0.23 Release Notes
...
- Every Computer MCP-owned Codex App Server now has a stable runtime identity,
  observable process-group ownership, and deterministic EOF, TERM, KILL, and
  reap behavior across normal close, replacement, timeout, shutdown, and
  parent death. The lifecycle never targets unrelated Codex clients.
...
- Supported Codex approval requests now flow through a durable, redacted
  broker that keeps gateway policy, agent approval, and user consent separate.
  Decisions include approve once, bounded session approval where allowed,
  denial, timeout, and restart-safe terminal receipts.
- Authorized Codex work can use governed Computer MCP tools and Git with
  workspace containment, reviewed mutation, hooks, operation tickets, and
  end-to-end request/result correlation. Push remains explicit and credentials
  remain outside tool results and logs.
- Stable official Codex Goal get, set, and clear bindings and turn steering are
  ...
  Computer MCP acceptance runs add durable criteria, evidence, pause states,
  budgets, stall detection, contradiction checks, and explicit completion.
- Exclusive and isolated-worktree leases prevent silent concurrent mutation.
  Reviewed managed worktrees retain parent/child lineage and reconcile only
  explicitly selected accepted evidence.
- App Server, Exec, MCP, approval, event, orchestration, diagnostics, and
  worktree inputs are bounded and redacted. Credential-like protocol IDs are
  represented by digest, and managed-root symlink replacement fails closed.
- The English and Simplified Chinese root manuals now lead with the product,
  trust model, quick start, Codex ownership choices, and honest limitations.
  This release is coordinated with the independent accessible static product
  site at `https://computer-mcp.github.io/`.

## Install

1. Download `Computer-MCP-1.0.23-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
...
- The production Bundle ID, Developer ID identity, private Keychain access
  ...
  credentials, workspaces, and policy remain compatible with 1.0.22. No data,
  credential, tunnel, CLI, MCP, or workspace migration is required.
