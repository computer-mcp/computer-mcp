# Computer MCP 1.0.24 Release Notes
...
1. Download `Computer-MCP-1.0.24-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
...
- The production Bundle ID, Developer ID identity, private Keychain access
  ...
  unpublished 1.0.23 candidate. No data, credential, tunnel, CLI, MCP, or
  workspace migration is required.
- The immutable signed `v1.0.23` candidate remains unpublished because its
  exact-artifact audit exposed the end-to-end timeout defect corrected here.
