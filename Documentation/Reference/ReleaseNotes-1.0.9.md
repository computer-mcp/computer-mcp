# Computer MCP 1.0.9 Release Notes
...
- Gateway connections now own and close their complete provider lifecycle.
  Disconnecting or stopping a Gateway releases Codex App Server, Exec, MCP,
  downstream MCP, HTTP, stdio, workspace access, and child-process resources.
- Reviewed Codex App Server methods use the typed `swift-codex` API while
  preserving method allowlists, workspace scope, policy, timeout, and error
  normalization.
- The independent full-catalog probe has a hard deadline and validates actual
  Full Shell and native Computer Use lifecycles. Its Computer Use surface runs
  in an isolated helper process so AppKit termination cannot silently end the
  validation command.
- Expected provider failures count as evidence only when the MCP response is
  structured, contains the expected stable error marker, and correlates to one
  failed audit decision.
- Development-candidate rehearsal covered all 287 advertised tools with
  287 semantic passes, 287 correlated audit rows, and zero retained Codex App
  Server or MCP child processes after disconnect. Publication repeats the
  fail-closed release acceptance against this exact notarized artifact.
...
1. Download `Computer-MCP-1.0.9-universal.dmg` and `SHA256SUMS` from the same
   GitHub Release.
