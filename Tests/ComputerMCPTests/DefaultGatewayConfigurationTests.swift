...
final class DefaultGatewayConfigurationTests {
  ...
  func testDefaultManifestDefinesCompleteExplicitProfileBoundaries() throws {
    ...
    #expect(operateCapabilities.contains("codex.app.elevation.request"))
  ...
  func testObserveCatalogIsReadOnlyAndOperateCatalogAddsReviewedWriteProviders() throws {
    ...
    #expect(operateNames.contains("codex.app.elevation.request"))
    #expect(!operateNames.contains("codex.app.elevation.approve"))
    #expect(!operateNames.contains("codex.app.thread.start"))
